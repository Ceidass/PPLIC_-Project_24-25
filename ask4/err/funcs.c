...
void yyerror(const char *s) {
    if (error_count < MAX_ERRORS) {
        ...
        errors[error_count].text = strdup(yytext);  // Remember to free this memory later
        errors[error_count].message = strdup(s);    // Remember to free this memory later
        error_count++;
    } else {
...
void print_errors() {
    for (int i = 0; i < error_count; i++) {
        printf("Error %d at line %d: %s near '%s'\n", 
               i + 1, errors[i].line, errors[i].message, errors[i].text);
    }
}
